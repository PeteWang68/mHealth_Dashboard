...
setwd("/data/sbdp/PHOENIX/GENERAL/BLS/7NE49/phone/raw/949iuit/calls")
...
for (file in file_list){
  ...
  if (exists("d")){
    temp_d <-read.table(file, header=TRUE, sep=",")
    d<-rbind(d, temp_d)
    rm(temp_d)
  }
...
head(d)
d$day <- d$UTC.time
d$day <- substr(d$day, 1, 10)


setwd("~/Documents")
x <- write.table(d,"merged_call_7NE49_10012019.txt")
data<-read.table("merged_call_7NE49_10012019.txt")
head(data)
...
```{r}
#Import the dataset
#data = read.table("./data/Copy of simulation2.txt")
...
total = full_data %>% 
  group_by(day, call.type) %>% 
  summarise(count = sum(callindex))

# Create the weekly data for the latest week
week_index = which(full_data$NumDays == full_data$NumDays[nrow(full_data)] - 6)[[1]]

week_data = full_data[week_index:nrow(full_data),]

# Create a weekday label
week_label = week_data %>% 
  group_by(NumDays, WeekDays) %>% 
  count() %>% as.data.frame() %>% select(WeekDays) %>% as.matrix()
```
...
max_miss = total %>%   
  filter(call.type == "Missed Call")  
max_missing = max(max_miss$count)
mean_missing = mean(max_miss$count)
min_missing = min(max_miss$count)
...
week_data_out = week_data %>%
  filter(call.type %in% c(".", "Outgoing Call")) %>% 
  group_by(NumDays) %>%
  summarise(count = sum(callindex)) 

week_data_miss = week_data %>%
  filter(call.type %in% c(".", "Missed Call")) %>% 
  group_by(NumDays) %>%
  summarise(count = sum(callindex))

# date = cbind(Date = c(1:7), count = c(1:7)) %>% as.data.frame()
pastweek = merge(week_data_in, week_data_out, by = "NumDays", all = TRUE) %>% 
  merge(., week_data_miss, by = "NumDays", all = TRUE) %>% 
  mutate(Incoming = count.x,
         Outgoing = count.y,
         Missed = count) %>% 
  select(NumDays, Incoming, Outgoing, Missed) %>% 
  gather(key = in_out_miss, value = count, Incoming:Missed)
pastweek[is.na(pastweek)] <- 0

renderPlot({ 
   if (input$type == "Max") {
pastweek %>% 
  ggplot(aes(x = NumDays, y = count, col = in_out_miss)) +
  geom_line(size = 1.5) +
  geom_line(aes(y = max_incoming, col = "Max Incoming"), size = 1.5) +
  geom_line(aes(y = max_outgoing, col = "Max Outgoing"), size = 1.5) +
  geom_line(aes(y = max_missing, col = "Max Missing"), size = 1.5) +
  geom_point(size = 3) +
  geom_text(aes(label = count), vjust = -0.7, size = 4, color = "black") +
  scale_x_discrete(limit = week_label) + 
...
pastweek %>% 
  ...
  geom_line(aes(y = mean_missing, col = "Mean Missing"), size = 1.5) +
  geom_point(size = 3) +
  geom_text(aes(label = count), vjust = -0.7, size = 4, color = "black") +
  scale_x_discrete(limit = week_label) + 
  theme(plot.title = element_text(face = "bold"),
...
pastweek %>% 
  ...
  geom_line(aes(y = min_missing, col = "Min Missing"), size = 1.5) +
  geom_point(size = 3) +
  geom_text(aes(label = count), vjust = -0.7, size = 4, color = "black") +
  scale_x_discrete(limit = week_label) + 
  theme(plot.title = element_text(face = "bold"),
...
percent_data %>% 
  group_by(Period, call.type) %>% 
  summarise(Percent = percent(n()/num)) %>% 
  ggplot(aes(x = Period, y = Percent, fill = call.type)) +
  ...
  scale_fill_discrete(labels = c("Incoming", "Outgoing", "Missing")) +
